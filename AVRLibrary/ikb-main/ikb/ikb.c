...
void ikey_scan(struct _key *key, int8_t KB_NUM_KEYS, int8_t KB_KEY_SCAN_COUNT_DEBOUNCE_MS)
{
    ...
    do
    {
        ...
        {
            if (++key[k].counter0 == KB_KEY_SCAN_COUNT_DEBOUNCE_MS)
            {
                key[k].counter0 = 0x00;
...
void ikey_parsing(struct _key *key, int8_t KB_NUM_KEYS)
{
    uint8_t k;
    int8_t numGroup;
...
void ikey_clear_all_flag(struct _key *key, int8_t KB_NUM_KEYS)//clear ReadyToRead
{
    for (int8_t i=0; i<KB_NUM_KEYS; i++)
    {
...
int8_t ikey_is_all_process_finished(struct _key *key, int8_t KB_NUM_KEYS)
{
    for (int8_t i=0; i<KB_NUM_KEYS; i++)
    {
...
//}
void ikb_flush(struct _key *key, int8_t KB_NUM_KEYS)
{
    ikey_clear_all_flag(key, KB_NUM_KEYS);
}
